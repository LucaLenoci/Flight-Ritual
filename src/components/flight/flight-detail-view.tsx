...
import { CardRevealSequence } from "../cards/card-reveal-sequence";
...
import type { CardUnlockResultDto, FlightDetailResponse, RunwayMomentDto } from "../../lib/api-types";
...
export function FlightDetailView({ flightNumber }: { flightNumber: string }) {
  ...
  const [cardUnlocks, setCardUnlocks] = useState<CardUnlockResultDto | null>(null);
  ...
  if (error && !data) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6">
        <p className="font-display text-2xl italic text-text-primary">Couldn&apos;t find that flight</p>
        <p className="mt-2 text-sm text-text-secondary">{error}</p>
        <Link
          href="/"
          className="mt-6 inline-block rounded-pill border border-border px-4 py-2 text-sm text-text-primary hover:bg-paper-100 dark:hover:bg-surface-raised"
        >
          Back to journeys
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      {data.degraded && (
        <div role="status" className="mb-6 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning">
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2">
        <Card className="p-6 sm:col-span-2">
          <p className="mb-4 font-mono text-xs uppercase tracking-eyebrow text-text-secondary">Journey Timeline</p>
        ...
        {data.flight.phase === "ARRIVED" && (
          <div className="sm:col-span-2">
            <SaveToLegacyButton flightId={data.flight.id} onSaved={setCardUnlocks} />
          </div>
        )}
      </div>

      <RunwayMomentOverlay moment={activeMoment} onDismiss={() => setActiveMoment(null)} />
      <CardRevealSequence result={cardUnlocks} onDone={() => setCardUnlocks(null)} />
    </div>
  );
}
